...
export default async function Home({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main>
    ...
    <section className="bg-[#f6f4ed] py-24 md:py-32"><div className="shell"><div className="mx-auto max-w-4xl text-center"><p className="eyebrow text-black/45">{lang === "da" ? "Anvendelser" : "Applications"}</p><h2 className="section-title mt-6">{lang === "da" ? "Hvornår giver droneinspektion værdi?" : "Where does drone inspection add value?"}</h2><p className="mt-6 text-lg leading-8 text-black/55">{lang === "da" ? "Fra løbende tilstandskontrol til dokumentation før en vigtig beslutning." : "From routine condition monitoring to evidence before an important decision."}</p></div><div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{applications.map((title,index)=>{const Icon=applicationIcons[index];return <article key={title} className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-black/10 bg-white/45 p-7 text-center transition hover:-translate-y-1 hover:bg-white"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-white shadow-sm"><Icon className="h-6 w-6 text-[#9b7b2f]"/></span><h3 className="mt-7 text-lg font-semibold">{title}</h3></article>})}</div></div></section>
    ...
    <section id="process" className="bg-[#0d1730] py-24 text-white md:py-32"><div className="shell"><div className="mx-auto max-w-4xl text-center"><p className="eyebrow text-[#cda64d]">{lang === "da" ? "Sådan virker det" : "How it works"}</p><h2 className="section-title mt-7">{lang === "da" ? "Tre enkle skridt til et bedre overblik." : "Three simple steps to a clearer view."}</h2><p className="mt-6 text-lg leading-8 text-white/55">{lang === "da" ? "Fra første afstemning til en rapport, jeres team kan arbejde videre med." : "From the first conversation to a report your team can act on."}</p></div><div className="relative mt-20 grid gap-10 md:grid-cols-3"><div className="absolute left-[16.67%] right-[16.67%] top-8 hidden border-t border-[#cda64d]/60 md:block"/>{threeSteps.map(([number,title,body])=><article key={number} className="relative text-center"><span className="relative z-10 mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#cda64d] text-lg font-bold text-[#0d1730]">{number}</span><h3 className="mt-7 text-2xl font-semibold">{title}</h3><p className="mx-auto mt-4 max-w-sm leading-7 text-white/55">{body}</p></article>)}</div><div className="mt-14 text-center"><Link href="#contact" className="inline-flex items-center gap-3 rounded-full bg-[#cda64d] px-7 py-4 font-semibold text-[#0d1730]">{lang === "da" ? "Bestil en inspektion" : "Request an inspection"}<ArrowRightIcon className="h-4"/></Link></div></div></section>
