...
export default async function Home({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main>
    <section className="grain relative min-h-[820px] overflow-hidden bg-[var(--ink)] text-white">
      ...
      <video className="hero-video absolute inset-0 h-full w-full object-cover opacity-65" autoPlay muted loop playsInline preload="metadata" aria-hidden="true"><source src={`${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/videos/offshore-wind-turbines.mp4`} type="video/mp4"/></video>
    ...
    <div className="ticker-shell border-y border-black/10 bg-[var(--sand)] py-5" aria-label={lang === "da" ? "AeroSight-løsninger" : "AeroSight solutions"}><div className="ticker-track">{[0, 1].map((group)=><div key={group} className="ticker-group" aria-hidden={group === 1}>{t.ribbon.map((item)=><span key={`${group}-${item}`} className="ticker-item"><span>{item}</span><span className="ticker-dot">●</span></span>)}</div>)}</div></div>
    ...
    <section className="bg-[var(--mist)] py-24 md:py-36"><div className="shell"><div className="grid gap-8 md:grid-cols-[.7fr_1.3fr]"><p className="eyebrow text-black/50">{t.upcomingLabel}</p><h2 className="section-title max-w-4xl">{t.upcomingTitle}</h2></div><div className="mt-16 grid gap-4 md:grid-cols-2">{t.upcoming.map((item)=><article key={item} className="flex min-h-[240px] flex-col justify-between rounded-3xl border border-black/10 bg-[var(--snow)] p-7 transition-transform duration-300 hover:-translate-y-1"><span className="eyebrow w-fit rounded-full bg-[var(--signal)] px-4 py-2 text-[var(--ink)]">{t.upcomingBadge}</span><h3 className="max-w-md text-3xl font-medium tracking-[-.04em]">{item}</h3></article>)}</div></div></section>
