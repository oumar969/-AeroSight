...
export function CookieBanner({ lang }: { lang: Lang }) {
  ...
  return <aside className="fixed bottom-3 left-3 right-3 z-50 mx-auto max-w-2xl rounded-2xl border border-black/10 bg-white p-5 shadow-2xl md:flex md:items-center md:gap-6">
    <p className="flex-1 text-sm leading-6 text-black/65">{lang === "da" ? "Vi bruger kun statistik-cookies med dit samtykke. Nødvendige funktioner virker altid." : "We only use analytics cookies with your consent. Essential functions always work."}</p>
