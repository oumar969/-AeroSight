...
export function Header({ lang }: { lang: Lang }) {
  ...
  return <header className="absolute inset-x-0 top-0 z-40 text-white">
    ...
    {open && <nav className="mx-3 rounded-b-2xl bg-[var(--ink)] p-7 text-2xl lg:hidden">{t.nav.map((item, i) => <Link key={item} onClick={() => setOpen(false)} className="block border-b border-white/20 py-4" href={`/${lang}/#${["solutions","process","industries","why"][i]}`}>{item}</Link>)}<Link href={`/${other}/`} className="mt-6 inline-block uppercase">{other}</Link></nav>}
  </header>;
}
